...
{"translations":[{"index":0,"translated_text":"你好"}]}"""
...
class SubtitleTranslator:
    ...
    def translate_segments(
        ...
        for start in range(0, len(segments), self.config.batch_size):
            ...
            translations = self._translate_batch_resilient(batch, start)
    ...
    def _translate_batch_resilient(self, batch: list[SubtitleSegment], offset: int) -> list[str]:
        try:
            return self._translate_batch_strict(batch, offset)
        except RuntimeError:
            if len(batch) == 1:
                raise

        midpoint = len(batch) // 2
        left = self._translate_batch_resilient(batch[:midpoint], offset)
        right = self._translate_batch_resilient(batch[midpoint:], offset + midpoint)
        return left + right

    def _translate_batch_strict(self, batch: list[SubtitleSegment], offset: int) -> list[str]:
        parsed = self._translate_batch(batch, offset)
        ...
        for item in parsed:
            if isinstance(item, dict):
                ...
                if item_index is not None:
                    try:
                        by_index[int(item_index)] = str(value).strip()
                        continue
                    except (TypeError, ValueError):
                        pass
                positional.append(str(value).strip())
            else:
                positional.append(str(item).strip())

        expected_indices = list(range(offset, offset + len(batch)))
        if by_index:
            missing = [index for index in expected_indices if not by_index.get(index)]
            if missing:
                raise RuntimeError(f"翻译返回缺少字幕索引：{missing}")
            return [by_index[index] for index in expected_indices]

        if len(positional) != len(batch):
            raise RuntimeError(f"翻译返回数量不匹配：输入 {len(batch)} 条，返回 {len(positional)} 条")
        return positional

    def _translate_batch(self, batch: list[SubtitleSegment], offset: int) -> list:
        payload_items = [
            {
                "index": offset + idx,
        ...
        user_prompt = (
            ...
            "返回格式必须是 {\"translations\":[{\"index\":0,\"translated_text\":\"...\"}]}。"
            "每一项必须保留输入的 index。"
            "translations 数组必须与输入数组长度完全一致，不要遗漏、合并或新增任何字幕。\n\n"
